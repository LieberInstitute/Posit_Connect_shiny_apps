...
## Load the data
posit_connect_file <- "/r_data/lcollado/Posit_Connect_shiny_apps/dlpfc/DLPFC_asd_raw-1/QC_probe_test_spe_raw_app.rds"
if (file.exists(posit_connect_file)) {
    ## Location for the https://conn1.libd.org/ server
    spe <- readRDS(posit_connect_file)
} else {
    spe <- readRDS(here::here("dlpfc", "DLPFC_asd_raw-1", "processed-data", "QC_probe_test_spe_raw_app.rds"))
}
