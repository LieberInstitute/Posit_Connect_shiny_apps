...
initial[["RowDataTable1"]] <- RowDataTable(Selected="SNAP25", SearchColumns = c("", "SNAP25", "", "", ""), PanelWidth=12L)

initial[["FeatureAssayPlot1"]] <- FeatureAssayPlot(DataBoxOpen=FALSE, PanelWidth=6L,
                                                   XAxis = "Column data", XAxisColumnData = "dx.sex",
                                                   #YAxisFeatureDynamicSource = T,
...
mytour =  data.frame(element=c("#Welcome",".navbar-custom-menu","#RowDataTable1","#RowDataTable1","#FeatureAssayPlot1","#WholeTissueDE1","#WholeTissueDE1","#FeatureAssayPlot2","#DomainRestrictedDE1","#DomainRestrictedDE2"),
                     intro=c("This page contains pseudobulk data summaries for the Thompson et al. 2026 SRT data.",
                             ...
                             "This is the row data table. This table is used to select features (genes) to generate pseudobulk violin plots (<i>FeatureAssayPlots</i>) and populate differential expression (DE) tables (<i>WholeTissueDE</i> and <i>DomainRestrictedDE</i>).",
                             "Use the column search bars to filter for features. You must click the row of the feature you want in order to select the entry and populate the linked plots and DE tables. Selected entry appears blue.<br><br><b>NOTICE:</b> Searching is case sensitive! Due to a bug, typing in lower case will break the app's connection from the server and require restarting the web page. Make sure gene symbols are uppercase!",
                             ...
                             "These DE results tables will show all of the features from the row data table search criteria (see both <i>SNAP25</i> and <i>SNAP25-AS1</i>). To restrict DE to a single feature use the column filters or fine-tune row data table search with regex.",
                             "This specific DE table summarizes the whole-tissue DE model results for both domain-SP and domain-CT, including the omnibus F-test statistics and post-hoc t-test significance for dx-sex group contrasts.",
                             ...
                             "This DE table summarizes the domain-restricted DE model results for domain-SP annotations, including the omnibus F-test statistics and post-hoc t-test significance for dx-sex-domain group contrasts.",
                             "This DE table summarizes the domain-restricted DE model results for domain-CT annotations, including the omnibus F-test statistics and post-hoc t-test significance for dx-sex-domain group contrasts."))
