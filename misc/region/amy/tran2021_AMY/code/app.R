...
## Load the data
posit_connect_file <- "/r_data/lcollado/Posit_Connect_shiny_apps/amy/tran2021_AMY/sce_amy_small.rds"
if (file.exists(posit_connect_file)) {
    ## Location for the https://conn1.libd.org/ server
    sce_small <- readRDS(posit_connect_file)
} else {
    sce_small <- readRDS(here::here("misc", "region", "amy", "tran2021_AMY", "processed-data", "sce_amy_small.rds"))
}

posit_connect_file1 <- "/r_data/lcollado/Posit_Connect_shiny_apps/amy/tran2021_AMY/cell_colors_amy.rds"
if (file.exists(posit_connect_file1)) {
    ## Location for the https://conn1.libd.org/ server
    cell_colors <- readRDS(posit_connect_file1)
} else {
    cell_colors <- readRDS(here::here("misc", "region", "amy", "tran2021_AMY", "processed-data", "cell_colors_amy.rds"))
}
...
initial[["ReducedDimensionPlot1"]] <- new("ReducedDimensionPlot", Type = "PCA_corrected", XAxis = 1L,
                                          ...
                                          ColorByFeatureDynamicSource = FALSE, ColorBySampleName = "donor1_AAACCCAAGCACGATG.1",
                                          ...
                                          CustomLabelsText = "donor1_AAACCCAAGCACGATG.1", FontSize = 1,
...
initial[["FeatureAssayPlot1"]] <- new("FeatureAssayPlot", Assay = "logcounts", XAxis = "Column data",
                                      ...
                                      CustomLabels = FALSE, CustomLabelsText = "donor1_AAACCCAAGCACGATG.1",
